...
class UserProfile(models.Model):
    ...
    )  # Used string 'Address' to prevent import errors if Address is defined below
    profile_picture = models.ImageField(
        upload_to="profile_pictures/", null=True, blank=True
    )
    status = models.CharField(
        max_length=20, default="Safe", help_text="Current Aggregate House Status"
    )

    # --- Firefighter Specific Fields ---
    station = models.ForeignKey(
        "FireStation", on_delete=models.SET_NULL, null=True, blank=True
    )
    ...
    team = models.CharField(
        max_length=50, null=True, blank=True, help_text="E.g. Alpha Squad, Truck 1 Crew"
    )
...
class DutyAssignment(models.Model):
    firefighter = models.ForeignKey(
        UserProfile, on_delete=models.CASCADE, related_name="assignments"
    )
...
class Sensor(models.Model):
    owner = models.ForeignKey(
        UserProfile, on_delete=models.CASCADE, related_name="sensors"
    )
    name = models.CharField(max_length=100)
    ...
    x_position = models.FloatField(
        null=True, blank=True, help_text="Horizontal % on floor plan"
    )
    y_position = models.FloatField(
        null=True, blank=True, help_text="Vertical % on floor plan"
    )
...
class Maintenance(models.Model):
    ...
    maintenance_type = models.CharField(
        max_length=20, choices=TYPE_CHOICES, default="HealthCheck"
    )
    frequency = models.CharField(
        max_length=20, choices=FRECUENCY_CHOICES, default="monthly"
    )
    ...
    )  # When the maintenance is planned
    actual_date = models.DateField(
        null=True, blank=True
    )  # When the maintenance was actually done
    technician_notes = models.TextField(
        null=True, blank=True, help_text="Notes from the technician after maintenance."
    )
    updated = models.DateTimeField(auto_now=True)
...
class Report(models.Model):
    ...
    status = models.CharField(
        max_length=50, choices=STATUS_CHOICES, default=STATUS_SYSTEM_DETECTED
    )
    timestamp = models.DateTimeField(auto_now_add=True)
    updated = models.DateTimeField(auto_now=True)
    ...
    trigger_gas_level = models.IntegerField(
        null=True, blank=True, help_text="Combined Gas/Smoke level"
    )
    station = models.ForeignKey(
        FireStation, on_delete=models.SET_NULL, null=True, blank=True
    )
    mobilized_team = models.ManyToManyField(
        UserProfile, related_name="incident_reports", blank=True
    )
    in_charge = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
...
class MaintenanceImage(models.Model):
    maintenance = models.ForeignKey(
        Maintenance, on_delete=models.CASCADE, related_name="images"
    )
...
class SatelliteHotspot(gis_models.Model):
    ...
    location = gis_models.PointField(
        srid=4326, help_text="Longitude/Latitude of the fire"
    )
