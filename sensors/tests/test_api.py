...
class ComprehensiveApiViewsTests(TestCase):
    def setUp(self):
        ...
        self.ff_user = User.objects.create_user(username="nabil_on_duty", password="password123")
        self.ff_profile = UserProfileFactory(user=self.ff_user, role="firefighter", station=self.station, phone_number="0198765432")
    ...
    def test_receive_data_empty_body(self):
        ...
        response = self.client.post(self.receive_url, "", content_type="application/json")
        self.assertEqual(response.status_code, 200)
    ...
    def test_receive_data_sensor_not_found(self):
        ...
        response = self.client.post(self.receive_url, json.dumps(payload), content_type="application/json")
    ...
    def test_receive_data_deduplication_branch(self, mock_predict):
        ...
        Report.objects.create(status="System Detected", address=self.address, trigger_sensor=self.sensor)
        
        payload = {"sensor_id": self.sensor.id}
        response = self.client.post(self.receive_url, json.dumps(payload), content_type="application/json")
        
        self.assertEqual(response.status_code, 200)
        self.assertEqual(Report.objects.count(), 1) # Proves it updated the existing one instead of creating a duplicate
    ...
    @patch("sensors.views.api.predictor.predict", return_value="Fire")
    def test_receive_data_full_fire_dispatch_with_staff(self, mock_predict, mock_hav, mock_sms, mock_channels, mock_async):
    ...
    @patch("sensors.views.api.predictor.predict", side_effect=Exception("Database failure"))
    ...
    def test_create_address_when_none_exists_fallback(self):
        ...
        self.assertEqual(self.user_profile.address.street, "Emergency Location (GPS Pin)")
