...
def find_nearest_station(victim_lat, victim_lng):
    ...
    for station in stations:
        if station.address.latitude and station.address.longitude:
            dist = haversine(victim_lat, victim_lng, station.address.latitude, station.address.longitude)
            if dist < min_dist:
                min_dist = dist
...
def send_sms_broadcast(phone_numbers, message_text):
    ...
    try:
        ...
        for number in phone_numbers:
            ...
            if clean_num.startswith('whatsapp:'): formatted_num = clean_num
            elif clean_num.startswith('+'): formatted_num = f'whatsapp:{clean_num}'
            elif clean_num.startswith('0'): formatted_num = f'whatsapp:+60{clean_num[1:]}'
            else: formatted_num = f'whatsapp:+60{clean_num}'
