...
class VirtualSensor:
    ...
    def update(self):
        ...
        if self.state == "Fire":
            ...
            if self.humidity < 60: self.humidity += 0.5
            elif self.humidity > 60: self.humidity -= 0.1
                
            if self.methane > 300: self.methane -= 50
            if self.lpg > 300: self.lpg -= 50
            if self.flame_val < 4095: self.flame_val += 50
...
while True:
    for s in sensors:
        ...
        try:
            ...
            print(f"[{s.id}-{s.location[:3]}] {status_txt} | Gas:{data['methane']} | Temp:{data['dht22_temp']} | Flame:{data['flame_val']}")
