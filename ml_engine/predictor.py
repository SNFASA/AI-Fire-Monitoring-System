...
class FirePredictor:
    ...
    def predict(self, methane, lpg, co, air_quality, flame_val, dht22_temp, humidity):
        ...
        try:
            ...
            # B. AI Confidence
            if fire_prob >= 0.75:
                add_log(f"   🔥 DECISION: [FIRE] AI Confidence ({prob_percent}%) ({model_name})\n")
                return "Fire"
            
            elif 0.40 <= fire_prob < 0.75:
                add_log(f"   ⚠️ DECISION: [WARNING] AI Confidence ({prob_percent}%) ({model_name})\n")
                return "Warning"
            ...
            add_log(f"   ✅ DECISION: [SAFE] Confidence low ({prob_percent}%) ({model_name})\n")
            return "Safe"
