...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        chief = UserProfileFactory(
            user__username="chief_fire",
            user__password="password123",
            role="firefighter",
            station=station,
        )
        self.stdout.write(
            f"✅ Created Chief Firefighter: 'chief_fire' (pass: password123)"
        )
        ...
        for i in range(10):
            u = UserProfileFactory(
                user__username=f"public_user_{i}",
                user__password="password123",
                role="public",
            )
            users.append(u)
        ...
        self.stdout.write(
            f"✅ Created {len(all_sensors)} Sensors & {len(all_sensors)*20} Logs"
        )
