...
def calculate_hdbms_score(y_true, y_pred, y_prob):
    ...
    w = {
        "acc": 0.167, "prec": 0.167, "rec": 0.25,
        "f1": 0.25, "auc": 0.083, "mae": 0.042, "rmse": 0.042,
    }
    ...
    score = (
        (w["acc"] * acc) + (w["prec"] * prec) + (w["rec"] * rec) + 
        (w["f1"] * f1) + (w["auc"] * auc_val) - (w["mae"] * mae) - (w["rmse"] * rmse)
    )
    return score
...
def run_hdbms_training_task():
    ...
    logs = SensorDataLog.objects.all().values(
        'methane', 'lpg', 'co', 'air_quality', 'flame_val', 'dht22_temp', 'humidity', 'status'
    )
    
    if not logs.exists() or logs.count() < 100:
        print(f"⚠️ [Django Q] Not enough data logs yet ({logs.count()}/100 minimum). Skipping training.")
        return False
    ...
    X = df[['methane', 'lpg', 'co', 'air_quality', 'flame_val', 'dht22_temp', 'humidity']]
    y = df["fire_label"]
    ...
    X_train, X_val, y_train, y_val = train_test_split(
        X_train_full, y_train_full, test_size=0.2, random_state=42, stratify=y_train_full if len(np.unique(y_train_full)) > 1 else None
    )
